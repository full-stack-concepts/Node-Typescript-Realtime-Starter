...
import { IClient, UserTokenObject, IUserDefinitions, IResponse } from "../../../src/shared/interfaces";
...
describe("Client Routes", () => {
	...
	describe("POST /register", () => {		
		...
		it("shoud respond", async () => {		
			const url:string = `${defaultURI}/api/client/register`;				
	...
	describe("POST /login", () => {
		...
		before( async () => {
			const url:string = `${defaultURI}/api/client/login`;	
	...
	describe("POST /findOne", () => {
		...
		before( async () => {
			const url:string = `${defaultURI}/api/client/findone`;	
		...
		it("should return a client with an email identifier", () => {
			expect(client.core.email).to.be.a('string').and.exist;
			expect(v.isEmail(client.core.email)).to.equal(true);
		});
		it("should return a client with an url identifier", () => expect(client.core.url).to.be.a('string').and.exist );
		it("should return an client with infractructure identifier", () => {
			expect(client.core.identifier).to.be.a('string').and.exist;		
			expect(v.isUUID(client.core.identifier)).to.equal(true);
		});
	...
	describe("POST /delete", () => {
		...
		before( async () => {
			const url:string = `${defaultURI}/api/client/delete`;	
		...
		it("should respond with a 200 status",  () => expect(result.response).to.have.status(200) );
		it("should return a JSON Object", () => expect(v.isJSON(result.body)).to.equal(true) );
		it("should return a JSON object with <isClientDeleted> property", () => expect(body).to.have.property('isClientDeleted').and.to.be.true );
	});
